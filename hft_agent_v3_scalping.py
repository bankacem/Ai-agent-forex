...
class RiskManager:
    ...
    ⚠️ هذه الطبقة كانت غائبة تماماً في النسخة الأصلية. تشمل:
      ...
      - ✅ إضافة (سكالبينج): وقف خسارة يتحرك لنقطة الدخول (breakeven) بعد ربح
        معيّن، ثم وقف متحرك (trailing stop) يقفل الأرباح تلقائياً وباستمرار
        مع تحرك السعر لصالح المركز — بدون أي تدخل يدوي.
    """
    ...
    def __init__(
        ...
        stop_loss_pct: float = 0.02,          # وقف خسارة أولي لكل صفقة
        max_symbol_exposure_pct: float = 0.25,  # أقصى انكشاف إجمالي لرمز واحد
        breakeven_trigger_pct: float = 0.006,   # الربح المطلوب لتفعيل نقل الوقف لنقطة الدخول
        breakeven_buffer_pct: float = 0.0008,   # هامش بسيط فوق الدخول (يغطي تكلفة السبريد تقريباً)
        trailing_distance_pct: float = 0.006,   # مسافة الوقف المتحرك خلف أعلى سعر تحقق
    ):
    ...
    def update_trailing_stop(self, pos: Dict, current_price: float) -> None:
        """
        ✅ منطق السكالبينج المطلوب:
        1) بمجرد أن يتحرك السعر لصالح المركز بنسبة >= breakeven_trigger_pct،
           يُنقل وقف الخسارة إلى نقطة الدخول + هامش بسيط (breakeven) — فلا يمكن
           للصفقة أن تتحول لخسارة بعد هذه اللحظة مهما انعكس السعر لاحقاً.
        2) بعدها، الوقف "يتبع" أعلى سعر تحقق باستمرار على مسافة trailing_distance_pct
           (يرتفع فقط، لا ينخفض أبداً) — فيقفل جزءاً أكبر من الربح تلقائياً
           كل ما استمر الاتجاه لصالح المركز، بدون أي تدخل يدوي.
        """
        ...
        if not pos.get("breakeven_active", False) and profit_pct >= self.breakeven_trigger_pct:
        ...
        if pos.get("breakeven_active", False):
            ...
            if trailing_stop > pos["stop_loss"]:
                pos["stop_loss"] = trailing_stop  # الوقف يرتفع فقط، لا يتراجع أبداً
...
class FixedHFTAgent:
    ...
    def _execute_buy(self, symbol: str, price: float, signal: Dict) -> Dict:
        with self.lock:
            ...
            pos = self.positions.setdefault(
                symbol, {"quantity": 0, "avg_price": 0, "stop_loss": stop_loss,
                         "highest_price": price, "breakeven_active": False}
            )
    ...
    def check_stop_losses(self):
        """✅ فحص وقف الخسارة على كل مركز مفتوح في كل دورة — بعد تحديث الوقف
        المتحرك/نقطة التعادل أولاً (منطق السكالبينج)."""
        with self.lock:
        ...
        for symbol in symbols_to_check:
            ...
            # ✅ تحديث الوقف (breakeven ثم trailing) قبل فحص التفعيل
            with self.lock:
                self.risk_manager.update_trailing_stop(pos, current_price)
                stop_price = pos["stop_loss"]

            if current_price <= stop_price:
                reason = "وقف متحرك (حماية أرباح)" if pos.get("breakeven_active") else "وقف خسارة أولي"
                self.logger.warning(f"⛔ تفعيل {reason} لـ {symbol} عند {current_price:.5f} (وقف: {stop_price:.5f})")
